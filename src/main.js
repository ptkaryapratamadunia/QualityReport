      ...
      <aside id="sidebar" class="fixed md:static inset-y-0 left-0 z-50 w-64 glass-panel m-0 md:m-4 p-4 flex flex-col h-screen md:h-[calc(100vh-2rem)] transform -translate-x-full md:translate-x-0 transition-all duration-300 ease-in-out overflow-hidden">
        <div class="flex justify-between items-start mb-8 px-2" id="sidebarHeader">
            ...
            <div class="flex items-center space-x-2 flex-shrink-0" id="sidebarToggleGroup">
                <!-- Theme Toggle Button -->
                <button id="themeToggleBtn" class="p-1.5 rounded-lg text-slate-400 hover:text-blue-500 hover:bg-slate-200 dark:hover:bg-slate-700/50 transition-all items-center justify-center mr-1">
  ...
  const updateThemeUI = (dark) => {
      ...
      Object.values(state.charts).forEach(chart => {
          if (chart) {
             ...
             const labelColor = dark ? '#fff' : '#1e293b'

             // Update axes
             if (chart.options.scales.x) {
                chart.options.scales.x.ticks.color = textColor
                chart.options.scales.x.grid.color = gridColor
                // Update axis title if exists
                if (chart.options.scales.x.title) chart.options.scales.x.title.color = dark ? '#fff' : '#1e293b' // Or specific color
             }
...
const renderCharts = () => {
  ...
  state.charts.monthly = new Chart(ctx1, {
    ...
    options: {
      ...
      plugins: {
        legend: { labels: { color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' } },
        datalabels: {
            anchor: 'end',
            align: 'top',
          ...
          plugins: {
              ...
              datalabels: {
                  ...
                  color: (context) => {
                      ...
                      return context.dataset.type === 'line' ? '#ef4444' : (isDark ? '#fff' : '#1e293b');
                  },
                  font: { weight: 'bold', size: 10 },
                  formatter: (value) => {
                      return typeof value === 'number' ? value.toFixed(2) : value;
...
const renderLineMonthlyCharts = () => {
  ...
  const titleHTML = `
    <div class="mb-6">
      ...
      <p class="text-slate-500 dark:text-slate-400 text-sm">Comparison of Quantity Inspected and NG Ratio for each production line</p>
    </div>
  `
        ...
        datasets: [
          {
            ...
            datalabels: {
              anchor: 'end',
              align: 'top',
              color: document.documentElement.classList.contains('dark') ? '#fff' : '#07090eff',
  ...
  lines.forEach(line => {
      ...
      new Chart(ctx, {
          ...
          options: {
              ...
              plugins: {
                  legend: {
                      labels: { color: document.documentElement.classList.contains('dark') ? '#ffffffff' : '#475569' }
                  },
                  tooltip: {
                      mode: 'index',
                      intersect: false
              ...
              scales: {
                  y: {
                      ...
                      display: true,
                      position: 'left',
                      title: { display: true, text: 'Total NG (lot)', color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' },
                      grid: { color: document.documentElement.classList.contains('dark') ? '#334155' : 'rgba(0,0,0,0.1)' },
                      ticks: { color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' }
                  },
                  y1: {
                      type: 'linear',
                      display: true,
                      position: 'right',
                      title: { display: true, text: 'Cumulative %', color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' },
                      min: 0,
                      max: 110,
                      grid: { display: false },
                      ticks: { 
  ...
  lines.forEach(line => {
      ...
      new Chart(ctx, {
          ...
          options: {
              ...
              plugins: {
                  legend: {
                      labels: { color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' }
                  },
                  tooltip: {
                      mode: 'index',
                      intersect: false
              ...
              scales: {
                  y: {
                      ...
                      display: true,
                      position: 'left',
                      title: { display: true, text: 'Qty NG (Lot)', color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' },
                      grid: { color: document.documentElement.classList.contains('dark') ? '#334155' : 'rgba(0,0,0,0.1)' },
                      ticks: { color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' }
                  },
                  y1: {
          ...
          options: {
              ...
              scales: {
                  y: {
                      ...
                      display: true,
                      position: 'left',
                      title: { display: true, text: 'NG (%)', color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' },
                      grid: { color: document.documentElement.classList.contains('dark') ? '#334155' : 'rgba(0,0,0,0.1)' },
                      ticks: { color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' }
                  },
                  y1: {
...
const renderMcNoChart = () => {
  ...
  const tableHTML = `
      <table class="w-full text-sm text-left">
          <thead class="text-xs text-slate-600 dark:text-slate-400 uppercase bg-slate-100 dark:bg-slate-800/30">
              <tr>
          ...
          <tbody>
              <tr class="border-b border-slate-200 dark:border-slate-700/50">
                  ...
                  <td class="py-3 px-4 text-slate-800 dark:text-white font-bold text-center">${totalInspLot.toFixed(2)}</td>
              </tr>
              <tr class="border-b border-slate-200 dark:border-slate-700/50">
                  ${mcNoData.tableData.map(row => 
                      `<td class="py-3 px-4 text-slate-600 dark:text-slate-400 text-center">${row.ngPercent.toFixed(2)}</td>`
                  ).join('')}
                  <td class="py-3 px-4 text-slate-800 dark:text-white font-bold text-center">${avgNgPercent.toFixed(2)}</td>
              </tr>
          </tbody>
      </table>
  `
      ...
      options: {
          ...
          scales: {
              y: {
                  ...
                  display: true,
                  position: 'left',
                  title: { display: true, text: 'Qty Inspected/Lot', color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' },
                  grid: { color: document.documentElement.classList.contains('dark') ? '#334155' : 'rgba(0,0,0,0.1)' },
                  ticks: { color: document.documentElement.classList.contains('dark') ? '#94a3b8' : '#475569' }
              },
              y1: {
...
const renderHousingHorn = () => {
  ...
  metrics.forEach(m => {
      const card = document.createElement('div')
      card.className = 'bg-white dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700/50'
      card.innerHTML = `
...
const renderRekap = () => {
  ...
  if (state.dateRange && state.dateRange.start && state.dateRange.end) {
      const subtitle = document.createElement('p')
      subtitle.className = 'text-slate-500 dark:text-slate-400 text-sm mb-6 text-center'
      subtitle.textContent = `Periode dari Tanggal: ${state.dateRange.start} sampai Tanggal: ${state.dateRange.end}`
      mainDiv.appendChild(subtitle)
  }
  ...
  rekapData.forEach(category => {
      ...
      catDiv.className = 'bg-slate-50 dark:bg-slate-800/30 rounded-lg p-4 border border-slate-200 dark:border-slate-700/50'
              ...
              <tbody>
                  ${category.defects.map((defect, idx) => `
                      <tr class="border-b border-slate-200 dark:border-slate-700/30">
                          ...
                          <td class="py-2 px-2 text-slate-700 dark:text-slate-300">${defect.name}</td>
                  ...
                  <tr class="bg-slate-100 dark:bg-slate-700/30 font-bold">
                      ...
                      <td class="py-2 px-2 text-slate-800 dark:text-white text-right">${category.totalNG.toFixed(2)}</td>
                  </tr>
              </tbody>
          </table>
      `
...
const renderDefectByPartNameTable = () => {
    ...
    rowHTML += `<td class="py-3 px-4 text-slate-600 dark:text-white text-center">${Object.keys(partNameGroups).length}</td>`
    rowHTML += `<td class="py-3 px-4 text-slate-600 dark:text-white">TOTAL</td>`
    
    activeDefects.forEach(defect => {
        rowHTML += `<td class="py-3 px-4 text-slate-600 dark:text-white text-right">${columnTotals[defect] || 0}</td>`
    })
    ...
    tbody.innerHTML = Object.keys(partNameSummary).map((partName, idx) => {
        ...
        return `
            <tr class="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                ...
                <td class="py-3 px-4 text-slate-600 dark:text-white">${data.custId}</td>
                <td class="py-3 px-4 text-slate-600 dark:text-white">${partName}</td>
                <td class="py-3 px-4 text-slate-600 dark:text-white text-right">${avgNgPercent.toFixed(4)}</td>
                ...
                <td class="py-3 px-4 text-slate-600 dark:text-white text-right">${data.mtlCount}</td>
            </tr>
        `
    }).join('') + `
        <tr class="bg-slate-700/30 font-bold border-t-2 border-slate-600">
            <td class="py-3 px-4 text-slate-600 dark:text-white text-center">${Object.keys(partNameSummary).length}</td>
            <td class="py-3 px-4 text-slate-600 dark:text-white">-</td>
            <td class="py-3 px-4 text-slate-600 dark:text-white">TOTAL</td>
            <td class="py-3 px-4 text-slate-600 dark:text-white text-right">${(totals.count > 0 ? (totals.sumNgPercent / totals.count) : 0).toFixed(4)}</td>
            ...
            <td class="py-3 px-4 text-slate-600 dark:text-white text-right">${totals.mtlCount}</td>
        </tr>
    `
}
        ...
        <div id="filterLineContent" class="hidden">
            ...
            ` : `
                <div class="glass-panel p-6 mb-6 border-l-4 border-blue-500">
                    <h3 class="text-slate-900 dark:text-white font-semibold text-lg mb-2">Filtering Data berdasarkan Line, Jenis NG dan PartName untuk menampilkan grafik harian</h3>
                ...
                <details class="mb-6 glass-panel">
                    <summary class="cursor-pointer p-4 text-slate-900 dark:text-white font-medium hover:bg-slate-700/30 transition-colors">
                ...
                <div class="glass-panel p-6 hidden" id="defectDailyChartContainer">
                    <h3 class="text-slate-800 dark:text-white font-medium mb-4" id="defectDailyChartTitle"></h3>
                    <div class="h-96">
                ...
                <div class="glass-panel p-6 mb-6 border-l-4 border-green-500 hidden" id="partNameFilterSection">
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm mb-4">Pilih PartName untuk melihat tabel detail harian dan rekapitulasi</p>
                    ...
                    <!-- Dropdown list -->
                    <div id="defectPartNameDropdown" class="hidden absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded max-h-60 overflow-y-auto shadow-lg">
                ...
                <details class="mb-6 glass-panel hidden" id="defectPartDetailTableContainer">
                    <summary class="cursor-pointer p-4 text-slate-800 dark:text-white font-medium hover:bg-slate-100 dark:hover:bg-slate-700/30 transition-colors">
...
const processDefectDailyData = (defectType) => {
    ...
    lineData.forEach(row => {
        ...
        const defectPcsValue = Number(row[defectType + '(pcs)']) || 0
        
        // Get Insp(Lot) or Insp(B/H) - same value
        const inspLotValue = Number(row['Insp(Lot)']) || 0
        const inspPcsValue = Number(row['QInspec']) || 0
        
        dailyGroups[dateKey].defectLot += defectLotValue
        dailyGroups[dateKey].defectPcs += defectPcsValue
        dailyGroups[dateKey].inspLot += inspLotValue
        dailyGroups[dateKey].inspPcs += inspPcsValue
    })
...
const processDefectPartData = () => {
    ...
    if (filteredData.length > 0) {
        ...
        detailArray = Object.values(detailData).map(d => ({
            ...d,
        ...
        summaryArray = Object.values(summaryData).map(d => ({
            ...d,
            ngPercent: d.inspLot > 0 ? (d.defectLot / d.inspLot) * 100 : 0
        })).sort((a, b) => b.defectLot - a.defectLot) // Sort by highest defect matches first
    }
...
const renderDefectPartTables = () => {
    ...
    const detailColHeaderPcs = document.getElementById('defectPartDetailColHeaderPcs')
    const detailPercentHeader = document.getElementById('defectPartDetailPercentHeader')
    const summaryColHeader = document.getElementById('defectPartSummaryColHeader')
    const summaryColHeaderPcs = document.getElementById('defectPartSummaryColHeaderPcs')
    const summaryPercentHeader = document.getElementById('defectPartSummaryPercentHeader')
    ...
    if (filteringState.selectedDefectType) {
        ...
        [detailColHeader, summaryColHeader].forEach(el => { if(el) el.textContent = `${defectName} (LOT)`; });
        [detailColHeaderPcs, summaryColHeaderPcs].forEach(el => { if(el) el.textContent = `${defectName} (pcs)`; });
        [detailPercentHeader, summaryPercentHeader].forEach(el => { if(el) el.textContent = `${defectName}NG_%`; });
        
        // Render Detail Table
        if (detailBody) {
             detailBody.innerHTML = filteringState.partDetailData.map(row => `
                <tr class="border-b border-slate-600 dark:border-slate-700/50 hover:bg-blue-50 dark:hover:bg-slate-700/30 transition-colors">
                    ...
                    <td class="py-2 px-3 text-slate-600 dark:text-slate-400 text-right font-medium">${row.ngPercent.toFixed(2)}%</td>
                </tr>
            `).join('')
        ...
        if (summaryBody) {
            summaryBody.innerHTML = filteringState.partSummaryData.map((row, index) => `
                <tr class="border-b border-slate-600 dark:border-slate-700/50 hover:bg-blue-50 dark:hover:bg-slate-700/30 transition-colors">
                    <td class="py-3 px-4 text-center text-slate-400 font-medium">${index + 1}</td>
                    <td class="py-3 px-4 text-slate-900 dark:text-slate-300 font-medium">${row.partName}</td>
                    ...
                    <td class="py-3 px-4 text-slate-600 dark:text-slate-400 text-right border-r-2 border-blue-500/30">${row.inspLot.toFixed(2)}</td>
                    <td class="py-3 px-4 text-slate-600 dark:text-slate-400 text-right font-medium">${row.ngPercent.toFixed(2)}%</td>
                </tr>
            `).join('')
