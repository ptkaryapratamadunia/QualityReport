...
export const processExcelFile = async (files) => {
  ...
  try {
    ...
    const cleanedData = cleanData(combinedData)
    ...
    if (cleanedData.length > 0) {
        const dates = cleanedData.map(row => row.DateObj).filter(d => d && !isNaN(d))
        if (dates.length > 0) {
    ...
    const metrics = calculateMetrics(cleanedData)
    const chartsData = prepareChartsData(cleanedData)
    const summaryTables = prepareSummaryTables(cleanedData)
    const trialData = prepareTrialData(cleanedData)
    
    return { cleanedData, metrics, chartsData, summaryTables, fileMetadata, dateRange, trialData }
  } catch (error) {
...
const cleanData = (data) => {
  ...
  let processed = data.map(row => {
    ...
    const hasData = Object.keys(newRow).some(k => newRow[k] !== undefined && newRow[k] !== null && String(newRow[k]).trim() !== '')
  ...
  processed = processed.map(row => {
    ...
    row['partName'] = row['PartName'] // Add lowercase version for consistency
...
const prepareTrialData = (allData) => {
  const trialData = allData.filter(r => r.isTrial)
  ...
  // 1. Metrics
  const totalInsp = trialData.reduce((sum, row) => sum + (Number(row['Insp(Lot)']) || 0), 0)
  const totalNG = trialData.reduce((sum, row) => sum + (Number(row['NG(Lot)']) || 0), 0)
  const totalOK = totalInsp - totalNG
  
  const validRows = trialData.filter(row => row['NG_%'] !== undefined && row['NG_%'] !== null)
  const sumNGPercent = validRows.reduce((sum, row) => sum + (Number(row['NG_%']) || 0), 0)
  const ngPercent = validRows.length > 0 ? (sumNGPercent / validRows.length) : 0
  ...
  // 2. Full Trial Data (ALL columns for collapsible table - matching download file)
  const fullTrialData = trialData.map(row => ({
      line: row['Line'],
      date: row['Date'],
      shift: row['Shift'],
      custId: row['Cust.ID'],
      ...
      noJig: row['NoJig'],
      noCard: row['NoCard'],
      stdLoad: row['Std Load'],
      noBH: row['NoBH_NoLotMTL'],
      noBak: row['NoBak'],
      keterangan: row['Keterangan'],
      mcNo: row['M/C No.'],
      inspBH: Number(row['Insp(B/H)']) || 0,
      ngBH: Number(row['NG(B/H)']) || 0,
      okBH: Number(row['OK(B/H)']) || 0,
      ngPercent: Number(row['NG_%']) || 0,
      okPcs: Number(row['OK(pcs)']) || 0,
      qtyNG: Number(row['Qty(NG)']) || 0,
      qInspec: Number(row['QInspec']) || 0,
      inspLot: Number(row['Insp(Lot)']) || 0,
      okLot: Number(row['OK(Lot)']) || 0,
      ngLot: Number(row['NG(Lot)']) || 0,
      // All NG defect columns (pcs)
      warna: Number(row['Warna(pcs)']) || 0,
      buram: Number(row['Buram(pcs)']) || 0,
      berbayang: Number(row['Berbayang(pcs)']) || 0,
      kotor: Number(row['Kotor(pcs)']) || 0,
      tdkTerplating: Number(row['Tdk Terplating(pcs)']) || 0,
      rontok: Number(row['Rontok/ Blister(pcs)']) || 0,
      tipis: Number(row['Tipis/ EE No Plating(pcs)']) || 0,
      flekKuning: Number(row['Flek Kuning(pcs)']) || 0,
      terbakar: Number(row['Terbakar(pcs)']) || 0,
      watermark: Number(row['Watermark(pcs)']) || 0,
      jigMark: Number(row['Jig Mark/ Renggang(pcs)']) || 0,
      lecet: Number(row['Lecet/ Scratch(pcs)']) || 0,
      seret: Number(row['Seret(pcs)']) || 0,
      flekHitam: Number(row['Flek Hitam(pcs)']) || 0,
      flekTangan: Number(row['Flek Tangan(pcs)']) || 0,
      belang: Number(row['Belang/ Dempet(pcs)']) || 0,
      bintik: Number(row['Bintik(pcs)']) || 0,
      kilap: Number(row['Kilap(pcs)']) || 0,
      tebal: Number(row['Tebal(pcs)']) || 0,
      flekPutih: Number(row['Flek Putih(pcs)']) || 0,
      spark: Number(row['Spark(pcs)']) || 0,
      kotorH: Number(row['Kotor H/ Oval(pcs)']) || 0,
      terkikis: Number(row['Terkikis/ Crack(pcs)']) || 0,
      dimensi: Number(row['Dimensi/ Penyok(pcs)']) || 0,
      mtl: Number(row['MTL/ SLipMelintir(pcs)']) || 0
  }))

  // 3. Rekap Data Trial (Grouped by unique combination of PartName AND Keterangan)
  const rekapByKey = {}
  trialData.forEach(row => {
      const partName = row['PartName'] || row['Part.ID']
      const keterangan = row['Keterangan'] || 'None'
      const key = `${partName}|||${keterangan}` // Use separator to create unique key
      ...
      if (row['NG_%'] !== undefined && row['NG_%'] !== null) {
          ...
          rekapByKey[key].ngPercentCount += 1
      }
  })

  const rekapRows = Object.values(rekapByKey).map(item => ({
  ...
  // Aggregate by PartName for defect types
  const defectByPart = {}
  trialData.forEach(row => {
      const partName = row['PartName'] || row['Part.ID']
      if (!defectByPart[partName]) {
          defectByPart[partName] = {}
          allNgColumns.forEach(col => defectByPart[partName][col] = 0)
      }
      allNgColumns.forEach(col => {
          defectByPart[partName][col] += (Number(row[col + '(pcs)']) || 0)
      })
  })
  
  // Filter out columns with all zeros
  const nonZeroColumns = allNgColumns.filter(col => {
      return Object.values(defectByPart).some(part => part[col] > 0)
  })
  
  const defectRekapRows = Object.entries(defectByPart).map(([partName, counts]) => {
      const row = { partName }
      let total = 0
      nonZeroColumns.forEach(col => {
          row[col] = counts[col]
          total += counts[col]
      })
      row.total = total
  ...
  // Defect rekap totals
  const defectRekapTotals = { partName: 'TOTAL' }
  let grandTotal = 0
  nonZeroColumns.forEach(col => {
      const sum = defectRekapRows.reduce((s, r) => s + (r[col] || 0), 0)
      defectRekapTotals[col] = sum
      grandTotal += sum
  })
  defectRekapTotals.total = grandTotal

  // 5. Charts Data
  const defectCounts = {}
  allNgColumns.forEach(col => defectCounts[col] = 0)
  
  trialData.forEach(row => {
      allNgColumns.forEach(col => {
          defectCounts[col] += (Number(row[col + '(pcs)']) || 0)
      })
  })
  ...
  const defectChart = {
      labels: sortedDefects.map(d => d.type + '(pcs)'),
      datasets: [{
          label: 'Total Qty NG (pcs)',
          data: sortedDefects.map(d => d.count),
          backgroundColor: '#ef4444',
          barThickness: 20
      }]
  }

  // Part Chart - Aggregate by PartName + Keterangan (same as rekap logic)
  const partAgg = {}
  trialData.forEach(row => {
      const partName = row['PartName'] || row['Part.ID']
      const keterangan = row['Keterangan'] || 'None'
      const key = `${partName}|||${keterangan}` // Same key as rekap
      
      if (!partAgg[key]) {
          partAgg[key] = { 
              label: keterangan !== 'None' ? `${partName} (${keterangan})` : partName,
              ok: 0, 
              ng: 0 
          }
      }
      partAgg[key].ok += (Number(row['OK(pcs)']) || 0)
      partAgg[key].ng += (Number(row['Qty(NG)']) || 0)
  })
  
  const sortedParts = Object.values(partAgg)
      .sort((a, b) => b.ng - a.ng)
      
  const partChart = {
      labels: sortedParts.map(p => p.label),
      datasets: [
          {
              label: 'Qty NG (pcs)',
              data: sortedParts.map(p => p.ng),
              backgroundColor: '#ef4444',
          },
          {
              label: 'Qty OK (pcs)',
              data: sortedParts.map(p => p.ok),
  ...
  return {
      metrics,
      fullData: fullTrialData,
      rekap: rekapRows,
      rekapTotals,
      defectRekap: defectRekapRows,
      defectRekapTotals,
      defectRekapColumns: nonZeroColumns,
      charts: {
          defects: defectChart,
          parts: partChart
      }
