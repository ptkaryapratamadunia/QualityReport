...
const renderMcNoChart = () => {
  ...
  const tableHTMLParts = [
    ...
    '      ' + mcNoData.tableData.map(row => '<td class="py-3 px-4 text-slate-400 text-center">' + row.inspLot.toFixed(2) + '</td>').join(''),
    '      <td class="py-3 px-4 text-white font-bold text-center">' + totalInspLot.toFixed(2) + '</td>',
    '    </tr>',
    '    <tr class="border-b border-slate-700/50">',
    '      ' + mcNoData.tableData.map(row => '<td class="py-3 px-4 text-slate-400 text-center">' + row.ngPercent.toFixed(2) + '</td>').join(''),
    '      <td class="py-3 px-4 text-white font-bold text-center">' + avgNgPercent.toFixed(2) + '</td>',
    '    </tr>',
    '  </tbody>',
    '</table>'
  ]
  const tableHTML = tableHTMLParts.join('\n')
...
const renderHousingHorn = () => {
  ...
  metrics.forEach(m => {
      ...
      card.innerHTML = `<div class="text-slate-400 text-xs mb-1">' + m.label + '</div><div class="text-white text-2xl font-bold">' + m.value + '</div>`
